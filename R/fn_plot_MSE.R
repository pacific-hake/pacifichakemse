...
fn_plot_MSE <- function(lst = NULL,
  ...
  stopifnot(!is.null(sim.data))
  stopifnot(!is.null(plotfolder))
  ...
  obj.plot <- map2(lst, nms, ~{
    tmp <- hake_objectives(.x, sim.data$SSB0, move = 1)
    tmp[[2]]$HCR <- .y
    tmp[[3]]$HCR <- .y
    tmp
  })
  df.obj <- map_df(obj.plot, ~{
    .x[[2]]
  })
  df.sp.vio <- map_df(obj.plot, ~{
    .x[[3]]
  })
  indicators <- unique(df.obj$indicator) %>% as.character
  ...
  p1 <- ggplot(df.obj2, aes(x = HCR,y = value)) +
  ...
  sp.vio.plot <- melt(df.sp.vio[df.sp.vio$year >2018,],id.vars = c("run","HCR","country","year"), measure.vars = 1:3,
  ...
  if(plotexp == TRUE){
    ...
    print(plot_grid(plotlist = list(p4,p5), ncol = 2, labels = c("a","b")))
    dev.off()
  }
