...
getParameters_OM <- function(trueparms = TRUE, mod = NA,df){
  if (trueparms){
    ...
    parms <- list(logRinit = pars$Value[which(nms == "SR_LN(R0)")],
                  ...
                  psel_fish = pars$Value[81:85], # Find a way to identify these
                  psel_surv = pars$Value[86:89],
                  initN = rev(pars$Value[ninit.idx]),
                  Rin = pars$Value[c(grep('Early_Recr', rownames(pars)),grep('Main_Recr', rownames(pars)))], # Perhaps should be 75 - check
