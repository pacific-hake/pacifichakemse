...
browser()
  for(yr in df$yrs){
    ...
    browser()
    for(season in seq_len(df$n_season)){
      for(space in seq_len(df$n_space)){
        ...
        if(E.temp/B.tmp >= .9){
          if(df$n_space[yr] < 2018){
            stop(paste('Catch exceeds available biomass in yrs:',df$yrs,' and season', season, 'area', space)) # Stop if in the past
          }
          #pr_int(paste('Catch exceeds available biomass in yrs:',df$yrs,' and season', season, 'area', space))
          E.temp <- 0.75*B.tmp
        ...
        if(season <df$n_season){
          N.save.age[,yr,space,season+1] <- N.save.age[,yr,space,season]*exp(-Z)-
            N.save.age[, yr,space,season]*exp(-Z)*(df$move_mat[space,,season,yr])+ # Remove the ones that leave
            N.save.age[, yr,spaceidx,season]*exp(-Z)*(df$move_mat[spaceidx,,season,yr])# add the ones come to the surrounding areas
          ...
          N.save.age[2:(n_age-1),yr+1,space,1] <- N.save.age[1:(n_age-2),yr,space,season]*exp(-Z[1:(n_age-2)])-
            N.save.age[1:(n_age-2), yr,space,season]*exp(-Z[1:(n_age-2)])*(df$move_mat[space,1:(n_age-2),season,yr])+ # Remove the ones that leave
            N.save.age[1:(n_age-2), yr,spaceidx,season]*exp(-Z[1:(n_age-2)])*(df$move_mat[spaceidx,1:(n_age-2),season,yr])# add the ones come to the surrounding areas

          # Plus group
          Nsurvive.plus <- (N.save.age[n_age-1, yr,space, df$n_season]*exp(-Z[n_age-1])+
                              N.save.age[n_age, yr,space, df$n_season]*exp(-Z[n_age]))
          Nout.plus <- Nsurvive.plus*(df$move_mat[space,n_age, season,yr]) # Leaving
          Nin.plus <- (N.save.age[n_age-1, yr,spaceidx,df$n_season]*exp(-Z[n_age-1])+
                         N.save.age[n_age, yr,spaceidx,df$n_season]*exp(-Z[n_age]))*
            (df$move_mat[spaceidx,n_age, season,yr]) # Incoming
