...
#'
#' @param ls.MSE A list of individual MSE runs. Should contain Biomass, Catch, and df with parameters
...
hake_objectives <- function(ls.MSE, SSB0, move = NA){
  ...
  names(ls.MSE) <- paste('run', 1:nruns, sep = '')
  df <- flatten(ls.MSE)
  ...
  if(is.na(move)){
    ...
    SSB.plot <-(df[names(df) == "SSB"] %>%  map(rowSums))#
    data.frame(SSB = as.numeric(rowSums(ls.MSE[[idx]]$SSB))/sum(SSB0), year = yr, run = paste('run',1, sep=''))
    names(SSB.plot) <- paste('run', 1:nruns, sep = '')
    mylist = t(do.call('rbind',SSB.plot))
